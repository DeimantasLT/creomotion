...
export const POST = withRole(['ADMIN', 'EDITOR'], async (request: NextRequest, user) => {
  try {
    ...
    const { name, email, company, phone, address, password } = validation.data;
    const existingClient = await prisma.client.findFirst({ where: { email: email.toLowerCase() } });
    if (existingClient) {
    ...
    const client = await prisma.client.create({
      data: { name, email: email.toLowerCase(), company, phone, address, passwordHash },
    });
